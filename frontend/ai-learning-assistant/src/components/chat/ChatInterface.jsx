...
const ChatInterface = () => {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4 h-[75vh]">
      {/* Chat Area */}
      <div className={`flex flex-col flex-1 bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-xl shadow-slate-200/50 overflow-hidden ${!showEditor ? 'lg:w-full' : ''}`}>
        {/* Chat Header for Toggling Editor */}
        <div className="flex items-center justify-between p-3 border-b border-slate-200/60 bg-gradient-to-br from-slate-50/50 to-white/50">
           <h3 className="text-sm font-semibold text-slate-800 flex items-center gap-2">
             <MessageSquare size={16} className="text-emerald-600"/> CodeDojo Assistant
           </h3>
           <button 
          ...
          {history.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center">
              ...
              <p className="text-sm text-slate-500 max-w-xs">
                Ask me anything about the document, or write some code and submit it!
              </p>
            </div>
          ) : (
        ...
        <div className="p-4 border-t border-slate-200/60 bg-white/80">
          <form onSubmit={handleSendMessage} className="flex items-center gap-3">
            <input
              ...
              placeholder="Ask a question or explain your code..."
            ...
            <button
              type="submit"
              disabled={loading || (!message.trim() && !code)}
      ...
      {showEditor && (
        <div className="hidden lg:flex flex-1 flex-col h-full animate-in fade-in slide-in-from-right-4 duration-300">
          <CodeEditor 
            code={code} 
            setCode={setCode} 
            language="javascript" 
            onRun={async (currentCode) => {
               ...
               // Trigger the actual form handler in a somewhat hacky but functional way
               setTimeout(() => {
